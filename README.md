...
### Step 4 — Initialise the database (from your browser)

**No Shell access needed.** The app has a built-in setup page.

1. Open the URL Render gave you (something like `https://lwp-crm.onrender.com`).
2. Sign in with your `ADMIN_PASSWORD`.
3. On first visit, the app automatically redirects you to `/setup.html` because the database is empty.
4. You see a page titled "Set up database" with a **Run setup** button.
5. Click **Run setup**. The button says "Running..." for a few seconds, then shows a green "Setup complete" message.
6. Click **Open the app**. You are now on the dashboard.

That's it. The setup page:
- Creates the 4 database tables (organisations, outreach_log, email_templates, settings)
- Seeds the 14 email templates (7 purposes × PT and EN)
- Is safe to run more than once. If you click it again by accident, it does no harm.

### Step 5 — Import your Excel CRM

1. In the app sidebar, click **Backup**.
2. Under "Import your Excel CRM", upload `LWP_Outreach_CRM.xlsx`.
3. You should see: `Imported 50 organisations. Skipped 0 duplicates and 7 placeholder rows.`
4. Click **Dashboard**. You now see your 14 tier-1 organisations, with 11 waiting for outreach.
